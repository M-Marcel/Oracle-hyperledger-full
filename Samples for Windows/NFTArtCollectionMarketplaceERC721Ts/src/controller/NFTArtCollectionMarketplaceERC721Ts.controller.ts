...
import { ArtCollection } from "../model/NFTArtCollectionMarketplaceERC721Ts.model";
...
export class NFTArtCollectionMarketplaceERC721TsController extends OchainController {
  private Ctx: Context;
  ...
  constructor(ctx: Context) {
	...
	public async init(adminList: ERC721TokenAdminAsset[]) {
		...
		await this.Ctx.ERC721Token.saveClassInfo(ArtCollection);
	...
	*/
	...
	public async createArtCollectionToken(tokenAsset: ArtCollection) {
	...
	public async getTokenObject<T extends OchainModel<any>>(tokenId: string): Promise<T> {
			...
			if (token.tokenName && token.assetType && token.assetType === 'otoken') {
					...
					switch (token.tokenName) {
						case 'artcollection':
								tokenAsset = new ArtCollection(token, false, true);
								return tokenAsset;
						default:
  ...
  public async executeQuery(query: string) {
	...
	public async buy(from_org_id: string, from_user_id: string, to_org_id: string, to_user_id: string, nonfungible_token_id: string, amount_paid: number)  {
		try { 
				const token = await this.Ctx.ERC721Token.get(nonfungible_token_id);
				const t = new ArtCollection(token);
				let msg = `Token ID : '${nonfungible_token_id}' has not been transferred'`;
				if (t.on_sale_flag==true) {
						if(t.price <= amount_paid) {
								...
								throw new Error(`Token ID : '${nonfungible_token_id}' has not been transferred to UserID : '${to_user_id}' as the amount was not fully paid'`);
						}
				}
				else {
						throw new Error(`Token ID : '${nonfungible_token_id}' is not posted for sell`)
				}
				return {msg};
		} catch(error) {
				throw new Error(error.message);
